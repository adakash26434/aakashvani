...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_tax_rates') {
        $fy        = trim($_POST['tax_fiscal_year'] ?? '2081/82');
        $vat       = (float)($_POST['vat_rate'] ?? 13);
        $single    = trim($_POST['income_tax_single']  ?? '');
        $married   = trim($_POST['income_tax_married'] ?? '');
        $corp      = trim($_POST['corporate_tax']      ?? '');
        $tds       = trim($_POST['tds_rates']          ?? '');
        $v2w       = trim($_POST['vehicle_tax_2w']     ?? '');
        $v4w       = trim($_POST['vehicle_tax_4w']     ?? '');
        $src       = trim($_POST['tax_source']         ?? 'Ministry of Finance, Nepal');
        $notes     = trim($_POST['tax_notes']          ?? '');
        // Validate JSON fields
        $jsonFields = ['income_tax_single'=>$single,'income_tax_married'=>$married,
                       'corporate_tax'=>$corp,'tds_rates'=>$tds,
                       'vehicle_tax_2w'=>$v2w,'vehicle_tax_4w'=>$v4w];
        foreach ($jsonFields as $k => $v) {
            if ($v !== '' && json_decode($v) === null) {
                $error = "Invalid JSON in field: $k"; break;
            }
        }
        if (!$error) {
            require_once __DIR__ . '/../functions.php';
            ...
            $pdo = db();
            // Check if row exists
            $exists = (int)$pdo->query("SELECT COUNT(*) FROM nepal_tax_info")->fetchColumn();
            if ($exists) {
                $stmt = $pdo->prepare("UPDATE nepal_tax_info SET
                    fiscal_year=?, vat_rate=?,
                    income_tax_single=?, income_tax_married=?, corporate_tax=?,
                    tds_rates=?, vehicle_tax_2w=?, vehicle_tax_4w=?,
                    updated_at=CURRENT_TIMESTAMP, updated_by='admin',
                    source=?, notes=?
                    WHERE id=(SELECT id FROM nepal_tax_info ORDER BY id DESC LIMIT 1)");
            } else {
                $stmt = $pdo->prepare("INSERT INTO nepal_tax_info
                    (fiscal_year,vat_rate,income_tax_single,income_tax_married,corporate_tax,
                     tds_rates,vehicle_tax_2w,vehicle_tax_4w,updated_by,source,notes)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?)");
            }
            $params = [$fy,$vat,$single,$married,$corp,$tds,$v2w,$v4w,'admin',$src,$notes];
            if (!$exists) $params = [$fy,$vat,$single,$married,$corp,$tds,$v2w,$v4w,'admin',$src,$notes];
            else          $params = [$fy,$vat,$single,$married,$corp,$tds,$v2w,$v4w,$src,$notes];
            $stmt->execute($params);
            flash('Tax rates updated successfully for FY ' . $fy);
            header('Location: /admin/settings.php#tax-rates'); exit;
        }
    }
    ...
    if ($action === 'save_settings') {
        ...
        $pwaName      = trim($_POST['pwa_name']       ?? (defined('SITE_NAME') ? SITE_NAME : 'आकाशवाणी'));
        $pwaShortName = trim($_POST['pwa_short_name'] ?? (defined('PWA_SHORT_NAME') ? PWA_SHORT_NAME : 'आकाशवाणी'));
        ...
        if (!$error) {
            ...
            // Read existing config.php and replace ONLY the defined() lines we are changing.
            // This preserves DB_DRIVER, CACHE_DIR, GOLD_CACHE_TTL, OPENAI_API_KEY, headers,
            // session bootstrap, timezone, and any future constants the admin form does not own.
            $cfgSrc = @file_get_contents($configPath);
            if ($cfgSrc === false) {
                $error = 'Could not read config.php.';
            } else {
                foreach ($overrides as $name => $value) {
                    $valLiteral = var_export($value, true);
                    $pattern = "/define\\(\\s*'" . preg_quote($name, '/') . "'\\s*,\\s*[^;]*\\);/";
                    if (preg_match($pattern, $cfgSrc)) {
                        $cfgSrc = preg_replace($pattern, "define('{$name}', {$valLiteral});", $cfgSrc, 1);
                    } else {
                        // append before closing PHP / end of file
                        $cfgSrc = rtrim($cfgSrc) . "\ndefine('{$name}', {$valLiteral});\n";
                    }
                }

                // Atomic write: temp file + rename, preserves perms; validates syntax first.
                $tmpPath = $configPath . '.tmp';
                if (file_put_contents($tmpPath, $cfgSrc) === false) {
                    $error = 'Could not write to config.php. Check file permissions (chmod 644).';
                } else {
                    // Syntax check
                    $syntaxOk = true; $syntaxMsg = '';
                    if (function_exists('shell_exec')) {
                        $out = @shell_exec('php -l ' . escapeshellarg($tmpPath) . ' 2>&1');
                        if ($out && stripos($out, 'No syntax errors') === false) {
                            $syntaxOk = false; $syntaxMsg = trim($out);
                        }
                    }
                    if (!$syntaxOk) {
                        @unlink($tmpPath);
                        $error = 'Refused to save: config.php would have a syntax error. ' . $syntaxMsg;
                    } else {
...
<!-- ═══════════ PWA SETTINGS ═══════════ -->
<a name="pwa"></a>
<div class="section" id="pwa-section">
  <div class="section-title">📱 PWA (App Install) Settings</div>
  <p class="text-sm text-slate-500 mb-5">यहाँ तपाईंको app को नाम set गर्नुस् — यूजरको mobile/desktop मा Home Screen मा यही नाम देखिन्छ।</p>
  <form method="POST" class="space-y-4">
    <input type="hidden" name="action" value="save_settings"/>
    <!-- Hidden fields to re-send existing values -->
    <input type="hidden" name="db_host"      value="<?= htmlspecialchars(defined('DB_HOST') ? DB_HOST : 'localhost') ?>"/>
    <input type="hidden" name="db_name"      value="<?= htmlspecialchars(defined('DB_NAME') ? DB_NAME : '') ?>"/>
    <input type="hidden" name="db_user"      value="<?= htmlspecialchars(defined('DB_USER') ? DB_USER : '') ?>"/>
    <input type="hidden" name="db_pass"      value="<?= htmlspecialchars(defined('DB_PASS') ? DB_PASS : '') ?>"/>
    <input type="hidden" name="site_url"     value="<?= htmlspecialchars(defined('SITE_URL') ? SITE_URL : '') ?>"/>
    <input type="hidden" name="site_email"   value="<?= htmlspecialchars(defined('SITE_EMAIL') ? SITE_EMAIL : '') ?>"/>
    <input type="hidden" name="whatsapp"     value="<?= htmlspecialchars(defined('WHATSAPP_NO') ? WHATSAPP_NO : '') ?>"/>
    <input type="hidden" name="site_name"    value="<?= htmlspecialchars(defined('SITE_NAME') ? SITE_NAME : '') ?>"/>
    <input type="hidden" name="site_tagline" value="<?= htmlspecialchars(defined('SITE_TAGLINE') ? SITE_TAGLINE : '') ?>"/>
    <input type="hidden" name="footer_about" value="<?= htmlspecialchars(defined('FOOTER_ABOUT') ? FOOTER_ABOUT : '') ?>"/>
    <div class="grid grid-cols-2 gap-4">
      <div>
        <label>App पूरो नाम <span style="color:#ef4444">*</span></label>
        <input type="text" name="pwa_name" required maxlength="60"
               value="<?= htmlspecialchars(defined('PWA_NAME') ? PWA_NAME : (defined('SITE_NAME') ? SITE_NAME : 'आकाशवाणी')) ?>"
               placeholder="आकाशवाणी"/>
        <p class="hint">Install prompt र splash screen मा देखिन्छ</p>
      </div>
      <div>
        <label>App छोटो नाम (Home Screen icon)</label>
        <input type="text" name="pwa_short_name" maxlength="12"
               value="<?= htmlspecialchars(defined('PWA_SHORT_NAME') ? PWA_SHORT_NAME : 'आकाशवाणी') ?>"
               placeholder="आकाशवाणी"/>
        <p class="hint">Icon तलको text — max 12 characters राम्रो</p>
      </div>
    </div>
    <div class="flex items-center justify-between pt-2">
      <a href="/admin/pwa-install.php" class="text-sm text-purple-600 font-bold hover:underline">
        📲 Install Guide हेर्नुस् →
      </a>
      <button type="submit" class="btn-primary">💾 Save PWA Settings</button>
    </div>
  </form>
</div>
<div class="section">
  ...
  <?php
    ...
    $taxInfo = getTaxInfo();
  ?>
  <form method="POST" class="section" id="tax-rates-section">
    <input type="hidden" name="action" value="save_tax_rates" />
    <div class="section-title">🇳🇵 Nepal Tax Rates (Budget अनुसार update गर्नुस्)</div>
    <p class="text-xs text-[#64748b] font-mono mb-5">
      हरेक वर्ष Nepal को Budget announce भएपछि यहाँबाट tax slabs update गर्नुस्। Tax calculator page मा automatically reflect हुन्छ।<br>
      Current FY: <strong class="text-[#16a34a]"><?= htmlspecialchars($taxInfo['fiscal_year'] ?? '2081/82') ?></strong> &nbsp;|&nbsp;
      Last updated: <strong><?= htmlspecialchars(substr($taxInfo['updated_at'] ?? '', 0, 10) ?: 'N/A') ?></strong> by <?= htmlspecialchars($taxInfo['updated_by'] ?? 'system') ?>
    </p>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
      <div>
        <label>आर्थिक वर्ष (Fiscal Year)</label>
        <input type="text" name="tax_fiscal_year" value="<?= htmlspecialchars($taxInfo['fiscal_year'] ?? '2081/82') ?>" placeholder="2081/82" />
        <p class="hint">e.g. 2081/82 — Budget announcement पछि update गर्नुस्</p>
      </div>
      <div>
        <label>VAT Rate (%)</label>
        <input type="number" name="vat_rate" step="0.01" value="<?= htmlspecialchars($taxInfo['vat_rate'] ?? '13') ?>" placeholder="13" />
        <p class="hint">मूल्य अभिवृद्धि कर — अहिले Nepal मा 13%</p>
      </div>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
      <div>
        <label>Income Tax — Single (JSON)</label>
        <textarea name="income_tax_single" rows="6" style="font-family:monospace;font-size:11px"><?= htmlspecialchars(json_encode($taxInfo['income_tax_single'] ?? [], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)) ?></textarea>
        <p class="hint">Array of {from, to, rate, label, label_en}. to=null for top slab.</p>
      </div>
      <div>
        <label>Income Tax — Married (JSON)</label>
        <textarea name="income_tax_married" rows="6" style="font-family:monospace;font-size:11px"><?= htmlspecialchars(json_encode($taxInfo['income_tax_married'] ?? [], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)) ?></textarea>
        <p class="hint">Married couple को लागि slab — अलग threshold हुन्छ।</p>
      </div>
      <div>
        <label>Vehicle Tax — 2 Wheeler (JSON)</label>
        <textarea name="vehicle_tax_2w" rows="6" style="font-family:monospace;font-size:11px"><?= htmlspecialchars(json_encode($taxInfo['vehicle_tax_2w'] ?? [], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)) ?></textarea>
        <p class="hint">Array of {cc_from, cc_to, rate, label}. cc_to=null for top band.</p>
      </div>
      <div>
        <label>Vehicle Tax — 4 Wheeler (JSON)</label>
        <textarea name="vehicle_tax_4w" rows="6" style="font-family:monospace;font-size:11px"><?= htmlspecialchars(json_encode($taxInfo['vehicle_tax_4w'] ?? [], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)) ?></textarea>
        <p class="hint">Engine CC अनुसार Bagmati Province base rates।</p>
      </div>
      <div>
        <label>Corporate Tax Rates (JSON)</label>
        <textarea name="corporate_tax" rows="5" style="font-family:monospace;font-size:11px"><?= htmlspecialchars(json_encode($taxInfo['corporate_tax'] ?? [], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)) ?></textarea>
      </div>
      <div>
        <label>TDS Rates (JSON)</label>
        <textarea name="tds_rates" rows="5" style="font-family:monospace;font-size:11px"><?= htmlspecialchars(json_encode($taxInfo['tds_rates'] ?? [], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)) ?></textarea>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
      <div>
        ...
        <input type="text" name="tax_source" value="<?= htmlspecialchars($taxInfo['source'] ?? 'Ministry of Finance, Nepal') ?>" placeholder="finance.gov.np" />
      </div>
      <div>
        <label>Notes (Admin Only)</label>
        <input type="text" name="tax_notes" value="<?= htmlspecialchars($taxInfo['notes'] ?? '') ?>" placeholder="Budget 2082/83 — Shrawan 1 देखि लागू" />
      </div>
    </div>

    <div class="flex items-center gap-4">
      <button type="submit" class="btn-primary" onclick="return confirm('Tax rates update गर्ने? Tax calculator मा तुरुन्तै reflect हुन्छ।')">
        💾 Update Tax Rates
      </button>
      <a href="/tax-calculator.php" target="_blank" class="text-xs text-[#7c3aed] hover:underline font-mono">
        → Tax Calculator हेर्नुस्
      </a>
    </div>
  </form>
